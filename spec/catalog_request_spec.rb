...
describe Vra::CatalogRequest do
  context 'when no subtenant ID is provided' do
    before(:each) do
      @request = @vra.catalog.request('catalog-12345',
                                      cpus: 2,
                                      memory: 1024,
                                      lease_days: 15,
    ...
    it 'uses the subtenant ID from the catalog item' do
      ...
      allow(@vra).to receive(:http_get).with('/catalog-service/api/consumer/catalogItems/catalog-12345').and_return(response)
  ...
  describe '#fetch_catalog_item' do
    context 'when the catalog item exists' do
      ...
      it 'calls http_get against the catalog_service' do
        expect(@vra).to receive(:http_get).with('/catalog-service/api/consumer/catalogItems/catalog-12345').and_return(response)
        @request.fetch_catalog_item
      end
    ...
    context 'when the catalog item does not exist' do
      it 'raises an exception' do
        allow(@vra).to receive(:http_get).with('/catalog-service/api/consumer/catalogItems/catalog-12345').and_raise(Vra::Exception::HTTPNotFound)
